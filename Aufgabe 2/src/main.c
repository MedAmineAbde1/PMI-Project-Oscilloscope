...
void draw_on_display()
{
    ...
    int8_t first_line_drawn = 0;
    int16_t current_line = 0;
    int16_t prev_line = 0;
    int32_t period_sum = 0;
    ...
    for (uint8_t i = 0; i < 240 ; i++)
    {
        ...
        if(((adc_array[(draw_i+239) % 240]) < 2048) && ((adc_array[(draw_i+240) % 240]) >= 2048) && (avg_drawn == 0))
        {
            ili9341_line_draw(i, 62, i, 132, ILI9341_COLOR_ORANGE);
            if (first_line_drawn == 1)
            {
                prev_line = current_line;
                current_line = i;
                if(lines_drawn == 1)
                {
                    period_sum = current_line - prev_line;
                }
            }
            first_line_drawn = 1;
            lines_drawn++;
            avg_drawn = 1;
        }else if(((adc_array[(draw_i+239) % 240]) > 1150) && ((adc_array[(draw_i+240) % 240]) <= 1150) && (avg_drawn == 1))
        {
            avg_drawn = 0;
        }

...
* @param ?
*/
void ili9341_draw_bmp_haha(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t* bmp_data, uint16_t color, uint16_t bgcolor)
{
    if (w%8 != 0)
    {
        w=w%8;
    ...
    for (size_t i = 0; i < h; i++)
    {
        for (size_t j = 0; j < w; j++)
        {
            for (size_t k = 0; k <= 7  ; k++)
            {
                if (bmp_data[i*j + j])
                {
                    ili9341_pixel_set(x+w, y+h, color);
                }
                else
                {
                    ili9341_pixel_set(x+w, y+h, bgcolor);
                } 
            }
        }
    }
...
void ili9341_draw_bmp_h(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t* bmp_data, uint16_t color, uint16_t bgcolor)
{
    // Check if the width is not a multiple of 8
    if (w % 8 != 0)
    {
        w = w / 8 + 1;
    }
    ...
    for (size_t i = 0; i < h; i++)
    {
        // Iterate over the bytes of each row
        for (size_t j = 0; j < w; j++)
        {
            // Iterate over each bit in the byte
            for (size_t k = 0; k <= 7; k++)
            {
                // Check if the k-th bit in the byte is set
                if (bmp_data[i * w + j] & (1 << (7 - k)))
                {
                    // Set the pixel to the foreground color
                    ili9341_pixel_set(x + j * 8 + k, y + i, color);
                }
                else
                {
                    // Set the pixel to the background color
                    ili9341_pixel_set(x + j * 8 + k, y + i, bgcolor);
                }
            }
        }
    }
